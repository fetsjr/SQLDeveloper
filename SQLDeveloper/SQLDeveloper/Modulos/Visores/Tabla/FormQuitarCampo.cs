...
namespace SQLDeveloper.Modulos.Visores.Tabla
{
    public partial class FormQuitarCampo : Form
    {
        string NombreTabla;
        CTabla Tabla;
        public FormQuitarCampo(string tabla)
        {
            NombreTabla = tabla;
        ...
        {
            FormUniques dlg = new FormUniques(Tabla.Nombre);
            dlg.ShowDialog();
        }
        ...
        private void BDeafult_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                DBProvider.DB.AlterTable_DropDefault(Tabla.Nombre, campo.DefaultName);
            }
            catch (System.Exception ex)
            {
                MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        {
            FormChecks dlg = new FormChecks(Tabla.Nombre);
            dlg.Show();
        }
    }
}
